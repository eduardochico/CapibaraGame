...
// ===== Juego (Canvas) — Desktop + Mobile, DPR, delta time =====
...
// Tiempo real
let running=false, gameOver=false, lastTs=0, t=0, speed=320; // px/seg inicial
let score=0, best=0;
const gravity = 1800; // px/seg^2
const jumpVelBase = -820; // px/seg (se ajusta por escala)

// Sprite del capibara — base64 del usuario
...
const pops = []; let spawnTimer=0; // segundos
...
let muted=false; function toggleMute(){ muted=!muted; AudioKit.mute(muted); if(btnMute) btnMute.textContent = muted? '🔈 Con sonido' : '🔇 Silencio'; }
...
function setMode(next, manual=false){
  ...
  if(btnToggle) btnToggle.textContent = mode==='mobile' ? '🖥️ Modo escritorio' : '📱 Modo móvil';
  resize(); reset();
...
function reset(){ running=false; gameOver=false; t=0; score=0; spawnTimer=0; pops.length=0; player.x = mode==='mobile'? Math.max(32, 0.12*(W/DPR)) : 120; player.w=Math.round(60*scale); player.h=Math.round(60*scale); player.y=groundY-player.h; player.vy=0; player.onGround=true; if(scoreEl) scoreEl.textContent='Puntuación: '+score; }
function start(){ AudioKit.unlock(); AudioKit.startMusic(); if(overlay) overlay.style.display='none'; if(gameOver) reset(); if(!running){ running=true; lastTs=performance.now(); requestAnimationFrame(loop); } }
function end(){ running=false; gameOver=true; best=Math.max(best,score); AudioKit.stopMusic(); if(overlay) overlay.style.display='grid'; if(card) card.innerHTML = `
      <h1>¡Game Over! 😵‍💫</h1>
      <p>Puntuación: <strong>${score}</strong> • Mejor: <strong>${best}</strong></p>
      <p>${mode==='mobile'?'Toca':'Presiona'} <span class="kbd">Espacio</span> para reintentar.</p>
...
function update(dt){
  ...
  const targetSpeed = 320 + Math.min(600, t*22);
  ...
  for(let i=pops.length-1;i>=0;i--){ const o=pops[i]; o.x -= speed*dt; const viewLeft=-20; if(o.x + o.w < viewLeft) pops.splice(i,1); if(collide({x:player.x,y:player.y,w:player.w,h:player.h}, o)){ if(!muted) AudioKit.beep({freq:120,dur:0.2,type:'sawtooth'}); end(); } }
...
(function runTests(){
  ...
  try { let vy=200, y=100-40-1, dt=1/60; vy+=1800*dt; y+=vy*dt; if(y+40>=100){y=100-40;vy=0;} (y===60&&vy===0)?ok('Física suelo'):ko('Física suelo'); } catch(e){ ko('Física suelo', e.message); }
  setTimeout(()=>{ (capyReady || (capyImg.complete && capyImg.naturalWidth>0))?ok('Sprite capy cargado'):ko('Sprite capy cargado','no está listo'); if(out) out.innerHTML = 'Tests: ' + results.map(x=>`<b>${x}</b>`).join(' · ').replaceAll('❌','<span class=\"fail\">❌</span>'); }, 150);
})();